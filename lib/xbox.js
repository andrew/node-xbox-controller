...
function XboxController(serialNumber) {
    ...
    if(serialNumber) {
      this.loadController(this.serialNumber);
    }
}

/*
These magic numbers are the current publicly registered device IDs for xbox 360 controllers

 045e  Microsoft
   028e Xbox 360 Controller
   028f Xbox 360 Wireless Controller (for completeness)
*/
function xboxDeviceFilter(hid) {
  var xboxDeviceIds = ['028e', '028f'];
  return !!~xboxDeviceIds.indexOf(hid.path.split('_')[2]);
}


XboxController.allControllers = function() {
  var devices = HID.devices().filter(xboxDeviceFilter);  
  return devices.map(function(d){ return new XboxController(d.serialNumber) });
}

var location,bufferStore = [],original=1,count=0;
...
XboxController.prototype.loadController = function(serialNumber) {
  var device = HID.devices().filter(function(d) { return d.serialNumber === serialNumber; })[0];
  ...
  if(device) {
    try {
      ...
      console.log( chalk.green('notice: '), 'Xbox controller (' + this.serialNumber + ') connected.' );
    }
    catch ( ex ) {
      console.log( chalk.red('error: '), device.serialNumber + ' has already been bound.' );
    }
  }
  else {
    console.log( chalk.red('error: '), 'Could not match a device to serial number.');
  }
};
